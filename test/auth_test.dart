...
void main() {
  group('Mock Authentication', (() {
    ...
    test(
      'Should not be Intialized at start',
      () {
    ...
    test(
      'User should be null at start',
      () {
        expect(provider.currentUser, null);
      },
    );
    test(
      'Initialization timeout of 2 seconds',
      () async {
        await provider.initialize();
        expect(provider.isInitialized, true);
      },
    ...
    test('Create user should delagate to login', () async {
      final badEmailUser =
          provider.createUser(email: 'foo@bar.com', password: 'password');
      expect(badEmailUser,
          throwsA(const TypeMatcher<UserNotFoundAuthException>()));
      final badPassWordUSer =
          provider.createUser(email: 'aa@aa.com', password: 'foobar');
    ...
    test('Logged in user gets verified', () {
      provider.sendEmailVerification();
      final user = provider.currentUser;
      expect(user, isNotNull);
      expect(user!.isEmailVerified, true);
    });
    test('Should be able to logout an login', () async {
